...
import { Search, Grid3X3, List, MapPin, Bookmark, BookmarkCheck, FileText, Plus, Package, Star, Filter, SlidersHorizontal, ChevronDown } from "lucide-react";
...
const TYPE_COLORS: Record<string, string> = {
  Sell: "bg-primary/10 text-primary border-primary/20",
...
const Resources = () => {
  ...
  const filtered = resources
    .filter((item) => {
      ...
      return matchSearch && matchCat && matchType && matchCondition;
    })
    .sort((a, b) => {
      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="sticky top-16 z-20 bg-background/95 backdrop-blur-md pb-3 pt-1 -mx-4 px-4 border-b border-border/30">
          ...
          <AnimatePresence>
            {showFilters && (
              ...
              >
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 p-3 bg-card rounded-xl border border-border/50">
                  <Select value={condition} onValueChange={setCondition}>
                    <SelectTrigger className="rounded-full text-xs h-8"><SelectValue placeholder="Condition" /></SelectTrigger>
                    <SelectContent>
                      {CONDITIONS.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <Select value={sortBy} onValueChange={setSortBy}>
                    <SelectTrigger className="rounded-full text-xs h-8"><SelectValue placeholder="Sort" /></SelectTrigger>
                    <SelectContent>
        ...
        {loading ? (
          ...
          <motion.div className="text-center py-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <p className="font-heading font-semibold text-lg text-foreground mb-1">No products found</p>
              ...
              {filtered.map((item, i) => (
                ...
                >
                  <Link to={`/resource/${item.id}`} className="block group">
                    <div className="bg-card rounded-xl overflow-hidden border border-border/30 hover:border-primary/30 hover:shadow-glow transition-all duration-300">
                      {/* Image */}
                      <div className="aspect-square bg-muted relative overflow-hidden">
                        ...
                        <div className="absolute top-2 left-2 flex flex-col gap-1">
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md border ${TYPE_COLORS[item.type] || "bg-muted text-muted-foreground"}`}>
                            {item.type}
                          </span>
                          {item.files && item.files.length > 0 && (
                            <span className="bg-card/90 backdrop-blur-sm text-[10px] font-medium px-2 py-0.5 rounded-md flex items-center gap-0.5 w-fit">
                        ...
                        {item.price === 0 && (
                          <div className="absolute bottom-2 left-2">
                            <span className="bg-primary text-primary-foreground text-[10px] font-bold px-2 py-0.5 rounded-md">FREE</span>
                          </div>
                        )}
                      </div>
                      ...
                      {/* Info */}
                      <div className="p-3">
                        <h3 className="font-medium text-sm line-clamp-2 leading-snug mb-1.5 group-hover:text-primary transition-colors min-h-[2.5rem]">
                          {item.title}
                        </h3>
                        {item.condition && (
                          <p className="text-[10px] text-muted-foreground mb-1.5">{item.condition} · {item.category}</p>
                        )}
                        <div className="flex items-end justify-between">
                          <div>
                            ...
                            {item.price > 0 && item.price < 500 && (
                              <p className="text-[10px] text-muted-foreground">Great deal</p>
                            )}
                          </div>
              ...
              >
                <Link to={`/resource/${item.id}`}>
                  <div className="bg-card rounded-xl p-3 flex gap-3 border border-border/30 hover:border-primary/30 hover:shadow-sm transition-all group">
                    <div className="h-20 w-20 bg-muted rounded-xl shrink-0 overflow-hidden">
                      {item.images && item.images.length > 0 ? (
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md border shrink-0 ${TYPE_COLORS[item.type] || ""}`}>
                          {item.type}
                        </span>
                        {item.condition && <span className="text-[10px] text-muted-foreground">{item.condition}</span>}
                      </div>
